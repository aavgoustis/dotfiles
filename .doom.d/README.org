...
* Xmonad config
Xmonad is a dynamically tiling X11 window manager written and configured in Haskell. The official documentation is located at https://xmonad.org

This is the Xmonad configuration of Aris Avgoustis.
My GitHub: https://github.com/aavgoustis/

* IMPORTS
These are Haskell modules that we must import so that we can use their options in the config.
...
* VARIABLES
Here we set variables for our font and for any other programs such as ~myBrowser~ and ~myEditor~ so we can change them easily and don't have to search for them in the config.
#+BEGIN_SRC haskell
myFont :: String
...
* STARTUP
Programs and scripts that we want Xmonad to execute at startup, such as running a compositor, setting a wallpaper, starting the system tray, starting the Emacs daemon, etc.
#+BEGIN_SRC haskell
myStartupHook :: X ()
myStartupHook = do
    spawnOnce "sh ~/.screenlayout/dual-monitor.sh &"
...
myWorkspaces = [" 1:dev ", " 2:www ", " 3:doc ", " 4:file ", " 5:vbox ", " 6:game ", " 7:mus ", " 8:vid ", " 9:gfx "]
...
myManageHook = composeAll
     ...
     , (className =? "firefox" <&&> resource =? "Dialog") --> doFloat -- Floating Firefox dialogs
...
myKeys =
    ...
    -- Emacs (CTRL-e followed by a key)
        ...
        -- , ("C-e e", spawn (emacsclient -c -a emacs))   -- emacs dashboard
