...
myStartupHook = do
    ...
    -- Trackpad
    spawnOnce "xinput --set-prop 'SynPS/2 Synaptics TouchPad' 'libinput Accel Profile Enabled' 0, 1"
    spawnOnce "xinput --set-prop 'SynPS/2 Synaptics TouchPad' 'libinput Natural Scrolling Enabled' 1"
    spawnOnce "xinput --set-prop 'SynPS/2 Synaptics TouchPad' 'libinput Tapping Enabled' 1"

    -- Mouse
    spawnOnce "xinput --set-prop 'Logitech Gaming Mouse G502' 'libinput Accel Profile Enabled' 0, 1"
...
myShowWNameTheme = def
    { swn_font              = "xft:SauceCodePro Nerd Font Mono:bold:size=60"
    , swn_fade              = 1.0
    , swn_bgcolor           = "#1c1f24"
    , swn_color             = "#ffffff"
    }
...
-- The layout hook
myLayoutHook = avoidStruts $ mouseResize $ windowArrange $ T.toggleLayouts tabs
...
myManageHook = composeAll
     ...
     , title =? "File Operation Progress" --> doFloat -- For Thunar progress window
...
myKeys =
    ...
    -- Xrandr
        , ("M-d d", spawn "sh ~/.screenlayout/dual-monitor.sh")
        , ("M-d n", spawn "sh ~/.screenlayout/notebook-only.sh")

    -- Rofi
        , ("M-p", spawn "rofi -modi drun,run -show drun -columns 4")
    ...
    -- Toggle them to hide and it sends them back to hidden workspace (NSP).
        ...
        , ("M-C-s m", namedScratchpadAction myScratchPads "mocp")
        , ("M-C-s c", namedScratchpadAction myScratchPads "calculator")
    ...
    -- Emacs
        , ("M-C-e e", spawn (myEmacs))   -- start emacs
        , ("M-C-e b", spawn (myEmacs ++ ("--eval '(ibuffer)'")))   -- list buffers
        , ("M-C-e d", spawn (myEmacs ++ ("--eval '(dired nil)'"))) -- dired
        , ("M-C-e i", spawn (myEmacs ++ ("--eval '(erc)'")))       -- erc irc client
        , ("M-C-e m", spawn (myEmacs ++ ("--eval '(mu4e)'")))      -- mu4e email
        , ("M-C-e n", spawn (myEmacs ++ ("--eval '(elfeed)'")))    -- elfeed rss
        , ("M-C-e s", spawn (myEmacs ++ ("--eval '(eshell)'")))    -- eshell
        , ("M-C-e t", spawn (myEmacs ++ ("--eval '(mastodon)'")))  -- mastodon.el
